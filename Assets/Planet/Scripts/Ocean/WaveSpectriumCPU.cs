...
namespace OceanSurfaceEffects
{
    ...
    public class WaveSpectrumCPU
    {
		...
		void PackResults()
		{
			...
			for (int x = 0; x < m_size; x++)
			{
				for (int y = 0; y < m_size; y++)
				{
					...
					m_result0[i].r = map0.x;
					m_result0[i].g = map0.y;
					m_result0[i].b = 0.0f;
					m_result0[i].a = 0.0f;
					
					m_result1[i].r = map1.x;
					m_result1[i].g = map1.y;
					m_result1[i].b = map1.z;
					m_result1[i].a = map1.w;
					
					m_result2[i].r = map2.x;
					m_result2[i].g = map2.y;
					m_result2[i].b = map2.z;
					m_result2[i].a = map2.w;
				}
			}
		}
		...
		public float SampleHeight(Vector3 worldPos)
		{
			...
			float offset = m_gridSizes.x / (m_fsize*2.0f);
			Vector2 pos = new Vector2(worldPos.x - offset, worldPos.z - offset) / m_gridSizes.x;
			ht += m_map0.GetPixelBilinear(pos.x, pos.y).r;
			
			offset = m_gridSizes.y / (m_fsize*2.0f);
			pos = new Vector2(worldPos.x - offset, worldPos.z - offset) / m_gridSizes.y;
			ht += m_map0.GetPixelBilinear(pos.x, pos.y).g;
